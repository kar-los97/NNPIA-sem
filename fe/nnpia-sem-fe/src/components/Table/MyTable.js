    ...
    return (
        <>
                                ...
                                <tr {...row.getRowProps()} className={"table-row"} key={i.toString()}>
                                    {row.cells.map((cell, i) => {
                                        return <td {...cell.getCellProps()}
                                                   ...
                                                   key={i.toString()}>{cell.render('Cell')}</td>
                                    })}
                                </tr>
                            )
                        })}
            ...
            <div className="d-flex flex-row justify-content-center">
                <button className={`rounded-circle ${canPreviousPage?'bg-info text-white':'bg-white text-black'} m-2`}
                ...
                {' '}
                <button className={`rounded-circle ${canNextPage?'bg-info text-white':'bg-white text-black'} m-2`}
                ...
                <span className={"form-label m-2"}>
                    Celkem {' '}
                    <strong>{data.length} záznamů </strong>{' '}
                </span>
                {' '}
